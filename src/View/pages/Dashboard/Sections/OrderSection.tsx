...
//Services:
import { getUserById } from "../../../../Model/Services/User/index";
import { getOrderById, getOrders, updateOrderState } from "../../../../Model/Services/Order/index";
...
export default function OrderSection() {
  ...
  const canAdvance = (order: Orders) => {
    if(role?.chef && role?.delivery) return order.state >= 1 && order.state <= 3;
  ...
  const getEditableStates = () => {
    if (role?.chef && role?.delivery) return [1, 2, 3, 4];
  ...
  const fetchOrders = async () => {
    setLoading(true);
    const { totalItems, orders } = await getOrders(page, limit, selectedState);
    setOrders(orders);
